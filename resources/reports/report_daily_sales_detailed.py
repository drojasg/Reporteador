...
class ReportDailySalesDetailedExcell(Resource):
    ...
    def post(self):
        ...
        query = reportsData.daily_sales_detailed(from_date = params['from_date'], to_date= params['to_date'], id_market_segment= params['iddef_market_segment'] , api= params['doc_type'])
...
class ReportDailySalesDetailedPDF(Resource):
    @api.representation(MIME)
    def post(self):
        params = request.get_json(force=True)
        params['doc_type'] = 'pdf'
...
class ReportDailySalesDetailedApi(Resource):
    @api.representation(MIME)
    def post(self):
        params = request.get_json(force=True)
        params['doc_type'] = 'api'
        ...
        xlsx_report = os.path.join(
            base.app_config("TMP_PATH"),
            "{}.xlsx".format("daily-sales-by-market", get_bulk(charsize=7)),
        )

        html_report = os.path.join(
            base.app_config("TMP_PATH"),
            "{}.html".format("daily-sales-by-market", get_bulk(charsize=7)),
        )

        pdf_report = os.path.join(
            base.app_config("TMP_PATH"),
            "{}.pdf".format("daily-sales-by-market", get_bulk(charsize=7)),
        )
