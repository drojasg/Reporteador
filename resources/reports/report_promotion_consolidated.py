...
class ReportPromotionConsolidatedExcell(Resource):
    ...
    def post(self):
        ...
        xlsx_report = os.path.join(
            ...
            "{}.xlsx".format("daily-sales-detailed", get_bulk(charsize=7)),
        )

        html_report = os.path.join(
            base.app_config("TMP_PATH"),
            "{}.html".format("daily-sales-detailed", get_bulk(charsize=7)),
        )

        pdf_report = os.path.join(
            base.app_config("TMP_PATH"),
            "{}.pdf".format("daily-sales-detailed", get_bulk(charsize=7)),
        )
        ...
        query = reportsData.promotion_consolidated(from_date = params['from_date'], to_date= params['to_date'], api= params['doc_type'])
        ...
        with pd.ExcelWriter(xlsx_report, engine='xlsxwriter') as writer:
            ...
            df.rename(columns=
            {
                'property_code':'Hotel',
                'code': 'Mercado',
                'promo_code_name': 'Nombre del Promo Code',
                'promo_code': 'Descripción Corta',
...
class ReportPromotionConsolidatedPDF(Resource):
    @api.representation(MIME)
    def post(self):
        params = request.get_json(force=True)
        params['doc_type'] = 'pdf'
...
class ReportPromotionConsolidatedApi(Resource):
    @api.representation(MIME)
    def post(self):
        params = request.get_json(force=True)
        params['doc_type'] = 'api'
        ...
        xlsx_report = os.path.join(
            base.app_config("TMP_PATH"),
            "{}.xlsx".format("promotion_consolidated", get_bulk(charsize=7)),
        )

        html_report = os.path.join(
            base.app_config("TMP_PATH"),
            "{}.html".format("promotion_consolidated", get_bulk(charsize=7)),
        )

        pdf_report = os.path.join(
            base.app_config("TMP_PATH"),
            "{}.pdf".format("promotion_consolidated", get_bulk(charsize=7)),
        )
