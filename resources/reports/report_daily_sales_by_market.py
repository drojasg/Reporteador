...
class ReportDailySalesByMarketExcell(Resource):
    ...
    def post(self):
        ...
        query = reportsData.daily_sales_by_market_and_date(from_date = params['from_date'], to_date= params['to_date'], api= params['doc_type'])
        ...
        with pd.ExcelWriter(xlsx_report, engine='xlsxwriter') as writer:
            ...
            df.rename(columns=
            {
                'iddef_market_segment':'Market Segment',
                'property_code' : 'Código propiedad',
                'code':'Code',
                'trade_name' : 'Propiedad',
                'total_booking_value':'Ingresos',
                'bookings': 'Reservas',
            ...
            title = 'Performance by visitor country'
...
class ReportDailySalesByMarketPDF(Resource):
    @api.representation(MIME)
    def post(self):
        params = request.get_json(force=True)
        params['doc_type'] = 'pdf'

        #print(params['doc_type'])
...
class ReportDailySalesByMarketApi(Resource):
    @api.representation(MIME)
    def post(self):
        params = request.get_json(force=True)
        params['doc_type'] = 'api'
