...
class ReportOnHoldConsolidatedExcell(Resource):
    ...
    def post(self):
        ...
        xlsx_report = os.path.join(
            ...
            "{}.xlsx".format("report_booking_on_hold_consolidated", get_bulk(charsize=7)),
        )

        html_report = os.path.join(
            base.app_config("TMP_PATH"),
            "{}.html".format("report_booking_on_hold_consolidated", get_bulk(charsize=7)),
        )

        pdf_report = os.path.join(
            base.app_config("TMP_PATH"),
            "{}.pdf".format("report_booking_on_hold_consolidated", get_bulk(charsize=7)),
        )
        ...
        query = reportsData.booking_on_hold_consolidated(from_date = params['from_date'], to_date= params['to_date'], api= params['doc_type'])
        ...
        with pd.ExcelWriter(xlsx_report, engine='xlsxwriter') as writer:
            ...
            {
                'code': 'Mercado / Pais',
                'property_code': 'Propiedad / Hotel',
                'bookings_onhold': 'Número de Bookings Onhold',
                'total_bookings_convert': 'Total de Ingresos Brutos',
                'convert_rate':'Tasa de Bookings convertidos',
                'total_room_nights': 'Total de Cuartos Noches',
                'bookings': 'Bookings',
                'avg_daily_rate': 'Tarifa promedio',
                'total_booking_value':'Total de Bookings Convertidos',
                'avg_los':'Estancias promedio',
            }
            )
            ...
            title = 'Bookings On Hold Consolidated'
...
class ReportOnHoldConsolidatedPDF(Resource):
    @api.representation(MIME)
    def post(self):
        params = request.get_json(force=True)
        params['doc_type'] = 'pdf'
...
class ReportOnHoldConsolidatedApi(Resource):
    @api.representation(MIME)
    def post(self):
        params = request.get_json(force=True)
        params['doc_type'] = 'api'
        ...
        with pd.ExcelWriter(xlsx_report, engine='xlsxwriter') as writer:
            ...
            df.rename(columns=
            {
                ...
                'property_code': 'Propiedad / Hotel',
                'bookings_onhold': 'Número de Bookings Onhold',
                'total_bookings_convert': 'Total de booking convertidos',
                'convert_rate':'Tasa de Bookings convertidos',
                'total_room_nights': 'Total de Cuartos Noches',
                'bookings': 'Bookings',
                'avg_daily_rate': 'Tarifa promedio',
                'total_booking_value':'Total de ingresos brutos',
                'avg_los':'Estancias promedio',
            }).to_excel(writer, startrow=1, sheet_name="Data", index=False, header = True)
