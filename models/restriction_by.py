...
class RestrictionBy(db.Model):
    __tablename__ = "def_restriction_by"

    iddef_restriction_by = db.Column(db.Integer, primary_key=True)
...
class RestrictionBySchema(ma.Schema):
    iddef_restriction_by = fields.Integer(dump_only=True)
...
class RestrictionByRefSchema(ma.Schema):
    iddef_restriction_by = fields.Integer(dump_only=True)
    code = fields.String(required=True, validate=validate.Length(max=45))
    estado = fields.Integer()
    usuario_creacion = fields.String(required=True, validate=validate.Length(max=45))
